...
// 		ft_echo(temp, false);
// 		// If the token is a pipe, break the loop
// 		if (temp->token == PIPE)
// 			break ;
// 		temp = temp->next;
// 	}
...
bool	builtin(t_statement *s, t_data *data)
{
	...
	if (streq(s->argv[0], "unset"))
		printf("call unset function here\n");
	...
	// If the command is "export", execute the export command and update the global exit status
		...
		printf("call export function here\n");
